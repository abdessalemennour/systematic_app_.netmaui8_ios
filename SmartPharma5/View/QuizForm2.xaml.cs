...
public partial class QuizForm2 : ContentPage
{
    ...
    public async Task Log(string actionType)
    {
        ...
        try
        {
            // Tentative de r�cup�ration de la localisation avec timeout
        ...
        {
            Console.WriteLine("Erreur de g�olocalisation (non bloquante) : " + ex.Message);
            // On continue sans les coordonn�es GPS
        }
        ...
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Requ�te SQL: " + sqlCmd); // Pour d�bogage
            }
            finally
            {
        ...
        {
            Console.WriteLine("Erreur g�n�rale lors de l'enregistrement du log : " + ex.Message);
        }
    }
    /**********************************************/
    private async void btnSave_Clicked(object sender, EventArgs e)
    {
        ...
        if (DbConnection.ConnectionIsTrue())
        {
            try
            {
                ...
                SuccessPopup.IsOpen = false;
                Partner_Form.UpdateMacAdresse(partner_Form.Id);
                // Nouveau: Appel de la m�thode de log
                try
                {
                    await Log("update"); // ou une autre actionType appropri�e
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Erreur lors du logging: " + ex.Message);
                    // Ne pas bloquer le flux m�me si le logging �choue
                }
                await App.Current.MainPage.Navigation.PopAsync();
            }
            catch (Exception ex)
    ...
    private async void OnActionButtonClickeddocument(object sender, EventArgs e)
    {
        ...
        try
        {
            var partnerform = this.partner_Form; // Assurez-vous que cette propri�t� est accessible
