...
namespace SmartPharma5.ViewModel
{
    class SammaryViewModel : BaseViewModel
    {
        ...
        public int GetTarget()
        {
            ...
            {
                int userId = Preferences.Get("iduser", 0);
                string pointToleranceQuery = $"SELECT point_tolerance FROM hr_employe WHERE user = {userId}";

                try
                {
                    MySqlCommand cmd = new MySqlCommand(pointToleranceQuery, DbConnection.con);
                    object result = cmd.ExecuteScalar();

                    if (result != null && result != DBNull.Value)
                    {
                        target = Convert.ToInt32(result); // Affecter la valeur récupérée
                    }
                }
                catch (Exception ex)
                {
                ...
                {
                    DbConnection.Deconnecter(); // Toujours fermer la connexion
                }
            }

            return target; // Retourner la valeur de point_tolerance
        }
